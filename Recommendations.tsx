...
import { Sparkles, TrendingUp, Clock, Flame, ChevronRight, RefreshCw } from 'lucide-react';
...
const Recommendations: React.FC<Props> = ({ onSelectRecipe }) => {
  ...
  const [dietFilter, setDietFilter] = useState(user?.goal === 'muscle-gain' ? 'Non-Veg' : 'Veg');
  ...
  const handleCourseFilter = (c: string) => {
    setCourseFilter(c);
    fetchRecommendations(c, dietFilter);
  };

  const handleDietFilter = (d: string) => {
    setDietFilter(d);
    fetchRecommendations(courseFilter, d);
  };
    ...
    <div className="max-w-5xl mx-auto p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-ink/50">
          Recipes recommended by our KNN model based on your health profile.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-ink text-paper rounded-2xl p-5">
          <p className="text-[10px] uppercase tracking-widest opacity-40 mb-2">Your BMI</p>
          <p className="font-serif font-bold text-4xl">{bmi.toFixed(1)}</p>
        ...
        <div className="bg-spice/5 border border-spice/10 rounded-2xl p-5">
          ...
          <p className={`font-serif font-bold text-2xl ${goalInfo.color}`}>
            {goalInfo.label}
          </p>
        ...
        <div className="bg-mint/5 border border-mint/10 rounded-2xl p-5">
          ...
          <p className="font-serif font-bold text-4xl text-mint">
            {data?.cal_target ?? '—'}
          </p>
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        ...
        <div className="flex gap-2">
          {['Veg', 'Non-Veg'].map(d => (
            <button
              key={d}
              onClick={() => handleDietFilter(d)}
              className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all border ${
                dietFilter === d
                  ? 'bg-spice text-paper border-spice'
                  : 'bg-white border-ink/10 text-ink/40 hover:border-spice/20'
              }`}
            >
              {d === 'Veg' ? '🌿 Veg' : '🍗 Non-Veg'}
            </button>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 opacity-30">
          ...
          <p className="text-sm mt-2">Try changing the filters above</p>
        </div>
      )}
      ...
      {data && (
        <div className="mt-8 p-4 bg-paper border border-ink/5 rounded-2xl text-center">
          <p className="text-[10px] text-ink/30 font-bold uppercase tracking-widest">
            Powered by KNN Recommender · {data.count} recipes matched to your profile ·
            BMI {data.bmi} · {data.goal.replace(/-/g, ' ')}
          </p>
        </div>
      )}
    </div>
